...
import { OutboundDNResponse, Page } from '../../core/models/api.types';
  ...
  template: `
    <div class="p-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Albaranes de salida</h1>
        <a routerLink="/outbound/new"
      ...
      <app-data-state [loading]="query.isPending()" [error]="query.isError() ? 'Error al cargar albaranes de salida' : undefined" [empty]="filteredItems().length === 0" [skeleton]="true" (retry)="query.refetch()">
        <app-table [columns]="columnDefs" [sortBy]="sortBy()" [sortDir]="sortDir()" (sortChange)="onSortChange($event)">
          @for (dn of filteredItems(); track dn.id) {
            <tr>
              ...
              <td class="text-gray-600 dark:text-gray-400">{{ dn.customerName ?? '—' }}</td>
...
export class OutboundListComponent {
  ...
  readonly query = injectQuery<Page<OutboundDNResponse>>(() => ({
    queryKey: ['outbound', { page: this.page(), q: this.q(), status: this.statusFilter() || undefined, sort: this.sortBy() ? `${this.sortBy()},${this.sortDir()}` : undefined }],
    queryFn: () => firstValueFrom(this.outboundService.list(this.page(), this.size, this.q() || undefined, this.statusFilter() || undefined, this.sortBy() ? `${this.sortBy()},${this.sortDir()}` : undefined)),
    staleTime: 30_000,
  ...
  protected exportToCsv(): void {
    ...
    exportCsv('albaranes-salida', ['Número', 'Cliente', 'Fecha', 'Estado', 'Total'], items.map((dn) => [
      dn.number, dn.customerName, dn.issueDate, dn.status, dn.totalAmount,
    ]));
  }
}
